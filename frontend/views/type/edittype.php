            ...
            <div class="row">
                <div class="col-lg-5">
                    <form  action="<?=Yii::$app->urlManager->createUrl(['type/edit'])?>" method="post">
                        <input type="hidden" name="type_id" value="<?=$info['type_id']?>">
                        <div class="form-group field-contactform-name required">
                            <label class="control-label" for="contactform-name">分类名称*</label>
                            <input type="text" class="form-control" name="type_name"  value="<?=$info['type_name']?>" autofocus aria-required="true">
                        </div>
                        ...
                        <div class="form-group field-contactform-subject required">
                            ...
                            <select class="form-control" name="status" aria-required="true">
                                ...
                                <?php foreach(Yii::$app->params['status'] as $key => $val){?>
                                    <option value="<?=$key?>"<?php if($info['status'] == $key){echo"selected";} ?>><?=$val?></option>
                                <?php }?>
                            </select>
                        </div>
